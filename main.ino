...
#define MAX_INPUT 90
#define SENSOR_POSITION_OFFSET 0
//         ######################################################################################
...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  ...
  if (millis() - time_ref > 20){
    time_ref = millis();
    float output = angle_x - SENSOR_POSITION_OFFSET;
    Serial.print("   Saída: ");
    Serial.print(output);
    Serial.print("     ");
    servo.write((int)input_from_pid(output, 0));
  }
  //         ##############################################################################################
...
float input_from_pid(float output, float setpoint) {
  static float error_past_1 = 0;
  float error = setpoint - output;
  Serial.print("Erro: ");
  Serial.print(error);
  Serial.print("        ");
  float Kp = 1, Ki = 10, Kd = 3;
  float input = Kp * error + Ki * (error_past_1 + error) + Kd * (error - error_past_1);
